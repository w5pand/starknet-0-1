...
mod SNErc721 {
    ...
    impl ERC721MetadataImpl of super::IERC721Metadata<ContractState> {
        ...
        fn token_uri(self: @ContractState, token_id: u256) -> Array<felt252> {
            ...
            let mut arrs = self._base_uri();
            if(token_id==0){
                arrs.append('0');
            }else{
                arrs.append(token_id.try_into().unwrap());
            }
            arrs
        }
    }
    ...
    #[external(v0)]
    impl ERC721MetadataCamelOnlyImpl of super::IERC721MetadataCamelOnly<ContractState> {
    ...
    #[generate_trait]
    #[external(v0)]
    impl SNERC721 of SNERC721Trait{
        fn mint(ref self: ContractState, to: ContractAddress){
            assert(self.nextId.read() < self.total_supply.read(), 'sold out');
            self._mint(to, self.nextId.read());
            self.nextId.write(self.nextId.read()+1);
        }
    ...
    impl InternalImpl of InternalTrait {
        ...
        fn _base_uri(self: @ContractState) -> Array<felt252>{
            ...
            arrs.append('https://ipfs.io/ipfs/');
            arrs.append('QmeSjSinHpPnmXmspMjwi');
            arrs.append('XyN6zS4E9zccariGR3jxcaWtq/');
            arrs
        }
    }
